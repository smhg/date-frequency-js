...
var util = {
  ...
  filter: function (unit) {
    ...
    if (_.indexOf(order.concat(['W']), unit) < 0) {
      return undefined;
    }
   ...
   */
  lower: function (unit) {
    return _.rest(order, _.indexOf(order, unit) + 1);
  },
  higher: function (unit) {
    return _.first(order, _.indexOf(order, unit));
  },
  between: function (left, right) {
    var lower = util.lower(left);
    return _.first(lower, _.indexOf(lower, right));
  }
};
